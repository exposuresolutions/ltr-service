...
class AbacusLTRModel:
    def __init__(self):
        ...
        self.model_id = os.getenv("ABACUS_MODEL_ID", "your-default-ltr-model-id")
        self.base_url = os.getenv("ABACUS_API_BASE_URL", "https://api.abacus.ai/v1")
    ...
    def predict_relevance(self, query: str, tool_description: str) -> float:
        ...
        try:
            ...
            # Extract the relevance score. Adjust this based on your model's output.
            relevance_score = prediction_data.get("relevance_score")
            if relevance_score is None:
                if "predictions" in prediction_data and isinstance(prediction_data["predictions"], list) and len(prediction_data["predictions"]) > 0:
                    ...
                    if isinstance(pred, dict) and "score" in pred:
                        relevance_score = pred["score"]
                    elif isinstance(pred, dict) and "relevance_score" in pred:
